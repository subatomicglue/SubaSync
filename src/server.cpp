...
void Server::do_accept(){
    acceptor_.async_accept([this](std::error_code ec, asio::ip::tcp::socket sock){
        if(!ec){
            log_info(nullptr, "Accepted connection from {}", sock.remote_endpoint().address().to_string());
            ...
            asio::async_read_until(sock, *buf, "\n",
                [sock = std::move(sock), buf](std::error_code ec, std::size_t bytes){
                    if(ec){ log_error(nullptr, "read failed: {}", ec.message()); return; }
                    std::istream is(buf.get());
                    std::string line;
                    std::getline(is, line);
                    log_info(nullptr, "Server received: {}", line);
                });
        } else {
            log_error(nullptr, "accept failed: {}", ec.message());
        }
        do_accept();
    });
}
